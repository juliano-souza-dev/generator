...
def _process_media_job() -> None:
    try:
        ...
        _process_update(percent=5, message="Baixando vídeo da fonte validada…", log="Download do vídeo iniciado com estratégias de fallback do yt-dlp.")
        ...
        _process_update(percent=36, message="Vídeo baixado.", log=f"Vídeo baixado · {_human_bytes(PROCESS_SOURCE_FILE.stat().st_size)}", level="success",
                        artifact=_artifact_payload("source", "original.mp4", "video", PROCESS_SOURCE_FILE, "Vídeo baixado da fonte validada"))
