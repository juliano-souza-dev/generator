...
def render_music_video(canonical: dict[str, Any], source_video: Path, output: Path) -> None:
    if not source_video.is_file(): raise RuntimeError("Vídeo fonte do Music não foi encontrado.")
    if shutil.which("ffmpeg") is None or shutil.which("ffprobe") is None: raise RuntimeError("FFmpeg/FFprobe não encontrado.")
    project=canonical.get("project") if isinstance(canonical.get("project"),dict) else {}
    media_duration=_media_duration_ms(source_video); source_start=int(project.get("source_video_start_ms") or project.get("scene_start_ms") or 0); source_end=int(project.get("source_video_end_ms") or project.get("scene_end_ms") or 0)
    if source_end>source_start and media_duration>=source_end-500: start,end=source_start,min(source_end,media_duration)
    else: start,end=0,min(media_duration,int(project.get("scene_duration_ms") or media_duration))
...
def _validate_hub_final_json(payload: dict[str, Any]) -> None:
    ...
    if str(kit.get("contentType") or "") == "music":
        if duration < 30000 or duration > 60000:
            raise RuntimeError(f"JSON final Music exige scene_duration_ms entre 30000 e 60000; recebeu {duration}.")
...
def generate_final_materials(
    ...
    *,
    ...
    if content_type == "music":
        if wants("music.video"):
            try:
                emit(89, "Gerando videoMusic vertical com legendas WbW em inglês…")
                render_music_video(canonical, Path(music_source_video or ""), music_video)
            except Exception as exc:
                music_video.unlink(missing_ok=True)
                fail("music.video", "videoMusic.mp4", exc)
        remember_artifact("mp4", "music_video", music_video)
